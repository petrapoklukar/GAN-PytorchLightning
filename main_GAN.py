...
from lightning_modules.GAN_Trainer import GAN
from data_modules.MNIST_GAN import MNIST_GAN_DataModule
import json
...
@app.command()
def train_in_debug(experiment_id:str='test', checkpoint_path:str=None):
    # Manual debug parameters
    TEST_limit_train_batches = 0.2
    ...
    # Init model
    gan_model = GAN(model_config, train_config, experiment_id)
    
    # Init data
    data_config = DataInput()
    data_model = MNIST_GAN_DataModule(data_config)
